...
public class BitTorrents {
    ...
    public static void main(String[] args) throws Exception {
       ...
       // BitTorrentInfo info=parse(baseUrl+"/torrent/EP60.torrent");
        ...
        System.out.println("Info信息:"+it.getName()+"\t"+it.getPiecesLength()+"\t"+it.getLength()+"\t"+it.getMd5sum()+"\t"+it.getPieces());

        if(info.getAnnounceList()!=null&&info.getAnnounceList().size()>0){
            for(String str:info.getAnnounceList()){  
                System.out.println("announce:"+str);
            }  
        }
        if(it.getPieces().length>0){
                System.out.println("getPieces:"+new String(it.getPieces()));
        }
        if(it.getFiles()!=null&&it.getFiles().size()>0){
